<template>
  <transition name="panel-slide" appear>
    ...
    >
      <div class="command-panel-container max-w-6xl mx-auto px-4 pointer-events-auto">
        <!-- Command Panel -->
        <div 
        ...
        <div class="panel-top-border h-1 w-full" :class="{ 'translating': exposedIsTranslating }"></div>
        
        <!-- Main Panel Content -->
        <div class="panel-content p-4 relative z-10" :class="{ 'content-expanded': exposedCurrentView !== VIEWS.MAIN }">
          <!-- MAIN VIEW -->
          <transition name="view-fade" mode="out-in">
            <div v-if="exposedCurrentView === VIEWS.MAIN" key="main" class="flex items-center justify-between gap-4">
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="status-group hidden sm:flex">
                  <div class="status-lights">
                    <div class="status-light active" :style="{ backgroundColor: themes[currentTheme].color }"></div>
                    <div class="status-light active" :style="{ backgroundColor: themes[currentTheme].color }"></div>
                ...
                <div class="status-group hidden md:flex">
                  <div class="signal-bars">
                    <div class="signal-bar active" :style="{ backgroundColor: themes[currentTheme].color }"></div>
                    <div class="signal-bar active" :style="{ backgroundColor: themes[currentTheme].color }"></div>
                    <div class="signal-bar active" :style="{ backgroundColor: themes[currentTheme].color }"></div>
                    <div class="signal-bar active" :style="{ backgroundColor: themes[currentTheme].color }"></div>
                  </div>
              ...
              <div class="flex items-center gap-2 flex-1 justify-center">
                ...
                <button @click="showThemeSelector" class="panel-selector-btn">
                  <div class="selector-icon-wrapper">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2l8 4.618v9.236l-8 4.618-8-4.618V6.618L12 2z" />
                    </svg>
                  </div>
                   <span class="text-xs font-bold tracking-wider font-orbitron" style="color: var(--primary-light)">
                      Kyber
                    </span>
                ...
                <button @click="showLanguageSelector" class="panel-selector-btn">
                  <div class="selector-icon-wrapper">
          ...
          <transition name="view-fade">
            <div v-if="exposedCurrentView === VIEWS.THEMES" class="view-container">
              <div class="view-header">
                <button @click="returnToMain" class="back-btn" :disabled="exposedIsThemeChanging">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                  </svg>
                </button>
                <h3 class="view-title">
                  {{ exposedIsThemeChanging ? t('theme.changing') : t('theme.title') }}
                  <span v-if="exposedIsThemeChanging" class="changing-indicator">
              ...
              <div class="options-grid">
                <button
                  ...
                  :class="{ 
                    ...
                    'disabled': exposedIsThemeChanging && currentTheme !== theme.id
                  }"
                  :disabled="exposedIsThemeChanging && currentTheme !== theme.id"
                >
                  <div class="lightsaber-preview" :style="{ backgroundColor: theme.color, boxShadow: `0 0 15px ${theme.color}` }"></div>
                  <div class="option-info">
                    <div class="option-name" :style="{ color: theme.color }">{{ t(`theme.themes.${theme.id}.name`) }}</div>
                    <div class="option-crystal" :style="{ color: theme.color, opacity: 0.8 }">{{ t(`theme.themes.${theme.id}.crystal`) }}</div>
                  </div>
                  <svg v-if="currentTheme === theme.id" xmlns="http://www.w3.org/2000/svg" class="check-icon" viewBox="0 0 20 20" fill="currentColor" :style="{ color: theme.color }">
          ...
          <transition name="view-fade">
            <div v-if="exposedCurrentView === VIEWS.LANGUAGES" class="view-container">
              <div class="view-header">
              ...
              <div class="options-grid">
                ...
                >
                  <div class="option-info flex-1">
                    <div class="option-name" style="color: var(--primary-color)">{{ t(`languages.${lang.code}`) }}</div>
                  </div>
                  <svg v-if="locale === lang.code" xmlns="http://www.w3.org/2000/svg" class="check-icon" viewBox="0 0 20 20" fill="currentColor" style="color: var(--primary-color)">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                  </svg>
                </button>
              </div>
...
.status-group:nth-child(1) { animation-delay: 0.5s; }
.status-group:nth-child(2) { animation-delay: 0.6s; }
.status-group:nth-child(3) { animation-delay: 0.7s; }
...
.signal-bar:nth-child(1) { @apply h-1.5; }
.signal-bar:nth-child(2) { @apply h-2.5; }
.signal-bar:nth-child(3) { @apply h-3.5; }
.signal-bar:nth-child(4) { @apply h-5; }
...
.option-card.active::after {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(circle at center, transparent 60%, var(--primary-color)10 100%);
  pointer-events: none;
}
