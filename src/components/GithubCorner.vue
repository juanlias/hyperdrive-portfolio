<template>
  <div class="github-corner-container">
    ...
    <!-- Backdrop -->
    <div v-if="isExpanded" class="panel-backdrop" @click="closePanel"></div>
  </div>
</template>
...
<script setup>
  ...
  const fetchGitHubStats = async () => {
    try {
      const response = await fetch(
        "https://api.github.com/repos/juanlias/hyperdrive-portfolio"
      );
  ...
  onMounted(() => {
    fetchGitHubStats();
    window.addEventListener("hyperdrive-easter-egg", handleEasterEgg);
  });

  onUnmounted(() => {
    window.removeEventListener("hyperdrive-easter-egg", handleEasterEgg);
  });
</script>
